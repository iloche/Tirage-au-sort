/*
Le mode strict de JavaScript a été introduit dans ECMAScript 5 (ECMAScript est le langage de programmation sur lequel est basé JavaScript). 
Depuis ECMAScript 5, le 'use-strict' est ajouté et il applique une analyse plus stricte et une gestion des erreurs sur le code au moment de l'exécution. 
...
Par exemple:
-> modification d'une variable en lecture seule (const)
-> x = 10; // Erreur : Assignation d'une valeur à une variable non déclarée
-> const let = 5; // Erreur : utilisation d'un mot réservé (let)
*/
'use strict'
...
et ce en évitant les erreurs potentiels sur les noms.
...
//Je déclare un objet qui contient trois propriétés avec leurs valeurs correspondantes.
const GAME_MODES = { election: "election", ranking: "classement", questions: "questions" }
/*
J'initialise dans une variable la valeur du mode de jeux que qui est selectionné par défaut. 
Dans ce cas, je vais rechercher la propriété election dans l'objet GAME_MODES.
Ce qui donne la même chose que ==> gameModeChoosed = "election"
*/
...
function nameOfPlayer() {
    ...
    let name;
    /*
    J'utilise dans dans ce cas une boucle "do while". Celle-ci est une structure de contrôle en programmation qui 
    permet d'exécuter un bloc de code au moins une fois, puis de répéter l'exécution du bloc tant qu'une condition 
    spécifiée est évaluée comme VRAIE.
    */
    do {
...
un for of (autres manières également possible pour la vérification). */
...
function checkToBegin(wantToPlay = true) {
    ...
    let minPlayer = 0;
    /*
    La structure de contrôle "switch case" permet de réaliser une sélection parmi plusieurs 
    options en fonction de la valeur d'une expression donnée. Elle offre une alternative plus concise et lisible 
    à une série de déclarations "if-else" imbriquées.
    */
    switch (gameModeChoosed) {
        ...
        default:
            /*
            Si gameModeChoosed ne correspond à aucune des valeurs des "case" précédentes, le code dans le default est exécuté. 
            Il affiche un message d'erreur indiquant que le mode de jeu choisi n'est pas reconnu.
            */
...
//Fonction appeler lorsque nous voulons débuter le jeux
function beginTheGame() {
    switch (gameModeChoosed) {
        ...
        case GAME_MODES.questions:
            ...
            // Nous demandons d'aller rechercher les h1 qui sont enfants direct de l'element qui a comme id "TITLE_MODE_ID_NAME"
...
function choosingAPlayer() {
    /*
    La méthode getElementsByClassName retourne une liste (ou une collection) d'éléments HTML qui possèdent la classe spécifiée. 
    Vous pouvez ensuite utiliser cette liste pour effectuer des manipulations sur les éléments
    */
    const playerAlreadySelected = document.getElementsByClassName(CATCH_PLAYER_CLASS_NAME);

    /*
    Sachant qu'une personne sera selectionné à la fois, je vérifie si dans à l'indice 0 de ma liste si il y a bien un element.
    Si c'est le cas, je remove la class "CATCH_PLAYER_CLASS_NAME", dans le cas contraire je ne fais rien
    */
...
function animatePlayerChoosed() {
    ...
    if (counterLoopAnimation < 6) {
        ...
        /* 
         clearInterval(intervalChoosed) arrête l'intervalle de temps spécifié par intervalChoosed, 
         qui a été précédemment créé à l'aide de setInterval. Cela permet de stopper l'exécution répétitive d'une action planifiée.
        */
...
function afterPlayerAnimated(playerName) {
    switch (gameModeChoosed) {
        ...
        case GAME_MODES.ranking:
            /* 
            Dans le cas où nous sommes en mode classement, nous cherchons le nom dans notre tableau playersPlayingArray 
            et le supprimons, car la personne a été choisie et nous n'avons plus besoin de l'avoir dans le tableau.
             */
...
function resetGame(gameIsFinish = true) {
    ...
    Nous vérifions donc à l'index 0 si nous avons un element.
    Si oui, alors nous le retirons*/
    if (playerAlreadySelected[0] != undefined) {
        playerAlreadySelected[0].classList.remove(CATCH_PLAYER_CLASS_NAME);
    }
...
function stopGame() {
    ...
    Ce qui me permet de rechercher tous les boutons, à l'exception de ceux qui ont les ID "ADD_PERSON_ID_NAME", "RETRY_ID_NAME" et "BTN_ELECTION_ID_NAME".
    */
...
//Fonction qui permet de créer la question/afficher pour le mode question
